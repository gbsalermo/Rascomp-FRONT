...
const initials = computed(() => {
  const name = registration.value?.robotNome?.trim() || 'Robô'
  return name
    .split(/\s+/)
    .slice(0, 2)
  ...
  <div class="page-stack follow-run-page" v-loading="loading">
    ...
    <section v-if="registration && config" class="follow-run-hero">
      <div class="follow-run-identity">
        <div class="follow-run-avatar" aria-label="Foto do robô ainda não cadastrada">
          <span>{{ initials }}</span>
        </div>
        <div>
          <span>Robô</span>
      ...
      <div class="follow-run-score">
        <div>
          ...
          <strong>{{ selectedTake }} / {{ config.numeroTomadas }}</strong>
        </div>
        <div>
        ...
        <div>
          <span>Tomadas ainda abertas</span>
          <strong>{{ remainingTakes }}</strong>
        </div>
        <div class="highlight">
          <span>Melhor tempo</span>
          <strong>{{ formatSeconds(currentRanking?.tempoFinalSegundos) }}</strong>
        </div>
      </div>
    </section>
    ...
    <nav v-if="config" class="follow-take-tabs" aria-label="Tomadas do robô">
      <button
        ...
        :key="tomada"
        type="button"
        :class="{ active: selectedTake === tomada }"
        @click="selectedTake = tomada"
      >
        <span>Tomada {{ tomada }}</span>
        <small>
          {{ robotAttempts.filter((item) => item.tomada === tomada).length }} / {{ config.tentativasPorTomada }} tentativas
        </small>
      </button>
    </nav>
    ...
    <section v-if="registration && config" class="follow-run-grid">
      <article class="follow-take-history">
        ...
        <div v-if="takeAttempts.length" class="follow-attempt-list">
          <div v-for="item in takeAttempts" :key="item.id" class="follow-attempt-item">
            <div class="follow-attempt-index">
              <strong>#{{ item.numeroTentativa }}</strong>
              <small>{{ formatDate(item.dataCadastro) }}</small>
            </div>
            <div class="follow-attempt-result">
              <strong>{{ formatSeconds(item.tempoFinalSegundos) }}</strong>
              <span>{{ formatSeconds(item.tempoSegundos) }} + {{ item.penalidadeSegundos || 0 }} s</span>
            </div>
            <div class="follow-attempt-meta">
              <span>{{ item.checkpointsAlcancados }} / {{ config.numeroCheckpoints }} checkpoints</span>
              <span v-if="item.observacao">{{ item.observacao }}</span>
            </div>
            <div class="follow-attempt-statuses">
              <el-tag :type="item.concluida ? 'success' : 'warning'" size="small" effect="light">
                {{ item.concluida ? 'Concluída' : 'Não concluída' }}
              </el-tag>
              <el-tag :type="item.valida ? 'success' : 'danger'" size="small" effect="light">
        ...
        <div v-else class="follow-empty-take">
          <strong>Nenhuma tentativa nesta tomada.</strong>
          <span>O próximo registro será a tentativa #1.</span>
        </div>
      </article>
      ...
      <aside class="follow-attempt-console">
        <div class="follow-run-card-heading">
          <div>
            <span class="eyebrow">Próxima passagem</span>
            <h2 v-if="nextAttemptNumber">Tentativa #{{ nextAttemptNumber }}</h2>
            <h2 v-else>Tomada completa</h2>
            <p class="muted">
              {{ nextAttemptNumber ? `Tomada ${selectedTake} de ${config.numeroTomadas}` : 'Selecione outra tomada disponível.' }}
            </p>
          </div>
        </div>
        ...
        <template v-if="nextAttemptNumber">
          <label class="follow-console-field">
            ...
            />
            <small>±10 s pelos controles; digitação livre para ajuste fino.</small>
          </label>

          <div class="follow-console-two">
            <label class="follow-console-field">
              <span>Penalidade (s)</span>
              <el-input-number v-model="attempt.penalidadeSegundos" :min="0" controls-position="right" />
            </label>
            <label class="follow-console-field">
          ...
          <div class="follow-console-note">
            Tempo máximo configurado: <strong>{{ config.maxTempoSegundos }} s</strong>.
            Se ultrapassar esse limite, o backend registra a tentativa como inválida.
          </div>

          <el-button class="brand-button follow-register-attempt" :loading="saving" @click="saveAttempt">
            Registrar tentativa #{{ nextAttemptNumber }}
          </el-button>
        </template>

        <div v-else class="follow-console-complete">
          <strong>Tomada {{ selectedTake }} concluída</strong>
          <span>Todas as {{ config.tentativasPorTomada }} tentativas previstas já foram registradas.</span>
        </div>
      </aside>
    </section>
  </div>
...
.follow-run-header h1 { margin: 2px 0 0; }
.follow-run-header p { margin: 2px 0 0; }
...
.follow-run-hero {
  ...
  box-shadow: 0 10px 30px rgba(65, 18, 42, .05);
}
...
.follow-run-avatar {
  ...
  box-shadow: 0 10px 24px rgba(79, 25, 103, .18);
  color: #fff;
  font-size: 28px;
  font-weight: 900;
}

.follow-run-identity > div:last-child {
  display: grid;
  gap: 3px;
}

.follow-run-identity span,
.follow-run-identity small {
  color: #83737b;
  font-size: 11px;
}

.follow-run-identity strong {
  color: #2e2127;
  font-size: 24px;
}
...
.follow-run-score span { color: #877780; font-size: 10px; }
.follow-run-score strong { color: #34262d; font-size: 21px; }
.follow-run-score .highlight { border-color: #e3b7c6; background: #fff3f7; }
.follow-run-score .highlight strong { color: #9f0f3b; }
...
.follow-take-tabs button.active {
  border-color: #9f0f3b;
  background: #fff3f7;
  color: #9f0f3b;
...
.follow-take-tabs span { font-size: 12px; font-weight: 850; }
.follow-take-tabs small { color: #8b7d84; font-size: 10px; }
...
.follow-take-history,
.follow-attempt-console {
  border: 1px solid #e8dde3;
  border-radius: 18px;
  background: #fff;
  box-shadow: 0 8px 26px rgba(58, 18, 39, .04);
}

.follow-take-history { padding: 18px; }
.follow-attempt-console { position: sticky; top: 18px; padding: 18px; }

.follow-run-card-heading {
  display: flex;
...
.follow-run-card-heading h2 { margin: 2px 0 0; }
.follow-run-card-heading p { margin: 3px 0 0; }

.follow-remaining-chip {
  padding: 7px 10px;
  border-radius: 999px;
  background: #f5edf1;
  color: #8f1238;
...
.follow-attempt-index,
.follow-attempt-result,
.follow-attempt-meta {
  display: grid;
  gap: 3px;
}

.follow-attempt-index strong { color: #9f0f3b; font-size: 18px; }
.follow-attempt-index small,
.follow-attempt-result span,
.follow-attempt-meta span { color: #887a81; font-size: 10px; }
.follow-attempt-result strong { color: #33262c; font-size: 16px; }
.follow-attempt-statuses { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 6px; }

.follow-empty-take,
.follow-console-complete {
  display: grid;
  place-items: center;
  gap: 5px;
  min-height: 150px;
  padding: 24px;
  border: 1px dashed #dacbd2;
  border-radius: 14px;
  background: #fcfafb;
  color: #776a71;
  text-align: center;
...
.follow-console-field small { color: #8d7f86; font-size: 10px; font-weight: 500; }
.follow-console-field .el-input-number { width: 100%; }
...
@media (max-width: 1100px) {
  .follow-run-hero { grid-template-columns: 1fr; }
  .follow-run-grid { grid-template-columns: 1fr; }
  .follow-attempt-console { position: static; }
}

@media (max-width: 760px) {
  .follow-run-header { grid-template-columns: 1fr; }
  .follow-run-score { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .follow-attempt-item { grid-template-columns: 58px 1fr; }
  .follow-attempt-meta,
  .follow-attempt-statuses { grid-column: 1 / -1; }
  .follow-attempt-statuses { justify-content: flex-start; }
  .follow-console-two { grid-template-columns: 1fr; }
}
</style>
